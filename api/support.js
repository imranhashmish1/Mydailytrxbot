  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (admin === "true") {
        ...
        const response = await fetch(
          `${supabaseUrl}/rest/v1/support_conversations?select=*&order=last_message_at.desc.nullslast`,
          {
            method: "GET",
            headers
          }
      ...
      if (conversation_id && String(telegram_chat_id) === ADMIN_TELEGRAM_ID) {
        const response = await fetch(
      ...
      const conversationResponse = await fetch(
        `${supabaseUrl}/rest/v1/support_conversations?telegram_chat_id=eq.${encodeURIComponent(
      ...
      const messagesResponse = await fetch(
        `${supabaseUrl}/rest/v1/support_messages?conversation_id=eq.${conversation.id}&select=*&order=created_at.asc`,
        {
          method: "GET",
          headers
        }
      ...
      await fetch(
        `${supabaseUrl}/rest/v1/support_messages?conversation_id=eq.${conversation.id}&sender_type=eq.admin&is_read=eq.false`,
        {
          method: "PATCH",
          headers,
          body: JSON.stringify({
      ...
      await fetch(
        `${supabaseUrl}/rest/v1/support_conversations?id=eq.${conversation.id}`,
        {
          method: "PATCH",
          headers,
          body: JSON.stringify({
    ...
    if (req.method === "POST") {
      ...
      if (!telegram_chat_id || !message_text) {
        return res.status(400).json({
          ok: false,
          message: "telegram_chat_id and message_text are required"
        });
      }
      ...
      const isAdmin = String(telegram_chat_id) === ADMIN_TELEGRAM_ID;
        ...
        await fetch(
          `${supabaseUrl}/rest/v1/support_conversations?id=eq.${conversation.id}`,
          {
            ...
            body: JSON.stringify({
              status: "open",
              unread_admin_count:
                Number(conversation.unread_admin_count || 0) + 1,
              last_message_at: new Date().toISOString()
            })
          }
        );
      ...
      if (sender_type === "admin") {
        ...
        if (!conversationResponse.ok || !conversations.length) {
        ...
        await fetch(
          `${supabaseUrl}/rest/v1/support_conversations?id=eq.${conversation.id}`,
          {
            ...
            body: JSON.stringify({
              status: "open",
              unread_user_count:
                Number(conversation.unread_user_count || 0) + 1,
